...
type PeringkatPajakKabChartProps = {
    data: PajakKabData | null;
    isLoading: boolean;
    selectedYear: number;
};
...
export function MergedDataCard({ data, selectedYear }: { data: PajakKabItem[], selectedYear:number }) {
    ...
    const handleDownload = async () => {
        ...
        if (png) {
            FileSaver.saveAs(png, 'peringkat-pajak-Kabupaten.png');
        }
    };
    ...
    return (
        <Card className="w-full">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    <CardTitle>Peringkat 10 Pajak Kabupaten Tahun {selectedYear} </CardTitle>
...
function PersentaseChart({ data, selectedYear }: { data: (PajakKabItem & { isTopPerformer: boolean })[], selectedYear:number }) {
    const [getPng, { ref, isLoading }] = useCurrentPng();
    const chartData = data.map(item => ({
        name: item.nama_daerah,
        persentase: item.persentase,
    ...
    const handleDownload = async () => {
        const png = await getPng();
        if (png) {
            FileSaver.saveAs(png, 'persentase-realisasi-pajak-Kabupaten.png');
        }
    };
    ...
    return (
        <Card className="w-full">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    <CardTitle>Persentase Realisasi Pajak Kabupaten Tahun {selectedYear}</CardTitle>
...
export function PeringkatPajakKabChart({ data, isLoading, selectedYear }: PeringkatPajakKabChartProps) {
    if (isLoading) {
